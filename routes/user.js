...
router.get("/", async function (req, res) {
  try {
    ...
    if (req.session.logged && req.session.user) {
      const cart = await db
        .collection(req.session.user._id)
        .find({ what: "cart" })
        .toArray();
...
router.get("/add-to-cart/:prodId/:userId", async (req, res) => {
  ...
  try {
    ...
    const item = await db
      .collection(userId)
      .findOne({ id: prodId, what: "cart" });
    if (item) {
      await db
        .collection(userId)
        .updateOne({ id: prodId, what: "cart" }, { $inc: { quantity: 1 } });
    } else {
      await db
        .collection(userId)
        .insertOne({ id: prodId, quantity: 1, what: "cart" });
    }
...
router.post("/place-order", async (req, res) => {
  ...
  try {
    ...
    if (details.length === 0) {
      return res
        .status(400)
        .json({ status: false, error: "No valid products in the cart." });
    }
    ...
    if (req.body.paymentMethod === "razorpay") {
      ...
      instance.orders.create(options, async (err, order) => {
        if (err) {
          ...
          return res
            .status(500)
            .json({ status: false, error: "Error creating Razorpay order." });
        }
      ...
      res.json({
        status: true,
        message: "Order placed successfully. Please await delivery."
      });
    }
  } catch (err) {
...
router.post("/payment-success", async (req, res) => {
  try {
    ...
    if (!paymentId || !orderId) {
      return res
        .status(400)
        .json({ status: false, error: "Payment ID or Order ID missing." });
    }
    ...
    if (paymentVerification.status === "captured") {
      ...
      await db
        .collection(req.session.user._id)
        .deleteMany({ what: "order", _id: new ObjectId(req.session.inserted) });
      await db
        .collection(req.session.adminId)
        .deleteMany({ what: "order", _id: new ObjectId(req.session.insert) });
    ...
    res
      .status(500)
      .json({ status: false, error: "Error during payment verification." });
  }
});
...
router.post("/payment-failure", async (req, res) => {
  ...
  res
    .status(500)
    .json({ status: false, message: "Payment failed. Please try again." });
});
...
router.get("/edit-order/:orderId", async (req, res) => {
  if (req.session.logged && req.session.user) {
    ...
    let order = await db
      .collection(req.session.user._id)
      .findOne({ what: "order", orderId: new ObjectId(req.params.orderId) });
    console.log(order);
...
router.post("/edit-order/:orderId", async (req, res) => {
  ...
  for (let admin of admins) {
    ...
    await db
      ...
      .updateOne(
        { orderId: new ObjectId(req.params.orderId) },
        { $set: { ...updatedFields, progress: "none" } }
      );
  }
...
router.get("/delete-order/:orderId", async (req, res) => {
  ...
  for (let admin of admins) {
    ...
    await db
      .collection(req.session.user._id)
      .deleteOne({ orderId: new ObjectId(req.params.orderId) });

...
router.get("/master-order-details/:adminId/:id", async (req, res) => {
  try {
    ...
    res.render('master/order-details',{ order,none:true,admin:req.params.adminId})
  } else if (order.progress === 'received') {
     res.render('master/order-details',{ order, received: true,admin:req.params.adminId})
  } else if (order.progress === 'shipped') {
     res.render('master/order-details',{ order,shipped: true,admin:req.params.adminId})
  } else if (order.progress === 'deliveried') {
     res.render('master/order-details',{ order,deliveried: true,admin:req.params.adminId})
  } 
  } catch (err) {
    console.error("Error loading cart:", err.message);
    res.status(500).send("Error loading cart.");
...
router.post('/master-update-order/:orderId/:adminId', async (req, res) => {
  ...
  await db.collection(req.params.adminId).updateOne(
    {_id: new ObjectId(orderId)},
    { $set : {progress: req.body.progress}})
  let order =  await db.collection(req.params.adminId).findOne(
...
router.post('/master-delete-order/:orderId/:adminId', async (req, res) => {
  ...
  let db = getDB()
  console.log(req.body)
  let admin = await db.collection(req.params.adminId).findOne({_id: new ObjectId(orderId)})
  await db.collection(req.params.adminId).deleteOne({_id: new ObjectId(orderId)})
  let de =  await db.collection(admin.user).deleteOne({_id: new ObjectId(orderId)})
  console.log(de)
  res.json({status:true})
});
...
router.get('/master-delete-orders/:adminId', async (req, res) => {
  ...
  for (const order of orders) {
    let de =  await db.collection(order.user).deleteOne({orderId: new ObjectId(order._id)})
    console.log(de)
  }
  res.json({status:true})
});
